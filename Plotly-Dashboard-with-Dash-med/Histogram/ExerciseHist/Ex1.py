...
df = pd.read_csv("./SourceData/abalone.csv")
print(df)

# create a data variable:
data = [go.Histogram(x=df["length"], name="Histogram",
                     xbins=dict(start=0, end=1, size=0.02))]


# add a layout
layout = go.Layout(title="Shell Weight")

# create a fig from data & layout, and plot the fig
fig = go.Figure(data=data, layout=layout)
pyo.plot(fig, filename="ex_hist.html")
